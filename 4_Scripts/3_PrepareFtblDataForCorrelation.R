s# Prepare Data for correlations
# Jonas Stillhard
# November 2017
...
wcSumH <- wc1960%>% 
  ...
  mutate(team=teamH, nDraws=nGamesP-(nWin+nLoss), nGamesKO=nLossesKO+nWinsKO, teamid=team1_id)

wcSumA <- wc1960%>% 
  group_by(year, teamA, team2_id) %>% 
  ...
  mutate(team=teamA, nDraws=nGamesP-(nWin+nLoss), nGamesKO=nLossesKO+nWinsKO, teamid=team2_id)

wcSum <- rbind(wcSumA, wcSumH)

wcSum <- wcSum %>%  
  group_by(year, team, teamid) %>% 
...
# Create a dataframe with all years and all teams that participated since 1962
wcSumAll <- expand.grid(unique(wcSum$year), unique(wcSum$team))
wcSumAll$teamid <- wcSum$teamid[match(wcSumAll$Var2, wcSum$team)]
wcSum$Participation <- 1

wcSumAll <- merge(wcSum, wcSumAll, by.x=c('year', 'team', 'teamid'), by.y=c('Var1','Var2', 'teamid'), all.y=T)

 

# Match world Bank Code to summarized data
  
wcSumAll$codeWB <- teams$idWB[match(wcSumAll$teamid, teams$team_id)]
wcSum$codeWB <- teams$idWB[match(wcSum$teamid, teams$team_id)]
...
saveRDS(wcSumAll, './1_data/WorldCupSummaryAll.RDS')
